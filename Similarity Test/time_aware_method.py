...
for news in news_stance_dataset:
    ...
    for s_news in similar_news_list:
        ...
        if s_news['title'] == news['title'] and s_news['media'] == news['source']:
            ...
            stance_score_dict[s_label] += 1 / (abs(candidate_media_bias-media_bias)+1)
            # if title == '國產疫苗跌不休！聯亞利空遭起底、高端EUA審查延後':
            #     print(s_news['media'],candidate_media_bias)
            #     print(s_news['stance'])
            #     print(stance_score_dict[s_label])
    # print('title: ',title)
    # print('label: ',label)
...
with open('proposed_error.json', 'w', encoding = 'utf-8') as jsonfile:
