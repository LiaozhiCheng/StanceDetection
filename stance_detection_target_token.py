...
model = BertForSequenceClassification.from_pretrained('ckiplab/albert-tiny-chinese', num_labels=3).to(device)
...
for epoch in range(num_epoch):
    ...
    for batch in tqdm(train_loader):
      ...
      # modify attention
      outputs = model.bert(input_ids=batch[0],token_type_ids=batch[1], attention_mask=batch[2])
      attention_scores = outputs[-1] # Get attention scores from the last layer
      print(batch[0])
      print('attention_scores:')
      # attention scores 跟著batch_size走 以這邊來說len(attention_scores) = 5
      print(len(attention_scores))

      # Access the attention scores for the first layer (layer 0)
      attention_layer0 = attention_scores[0]
      print('attention_layer0')
      print(len(attention_layer0))
      #print(attention_layer0)
      print(batch[0][0])

      # Print the attention scores for each token in the input
      tokens = tokenizer.convert_ids_to_tokens(batch[0][0].squeeze().tolist())
      for i, (token, score) in enumerate(zip(tokens, attention_layer0)):
        print(f"Token: {token}\tAttention Score: {score.item()}")
      break
...
with torch.no_grad():
  for batch in test_loader:
    ...
    # # modify attention
    # outputs = model.bert(input_ids=batch[0].to(device),token_type_ids=batch[1].to(device), attention_mask=batch[2].to(device))
    # attention_scores = outputs[-1] # Get attention scores from the last layer

    # # Access the attention scores for the first layer (layer 0)

    # attention_layer0 = attention_scores[-1]
